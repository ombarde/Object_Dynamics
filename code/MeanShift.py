...
num_frames = 5
object_roi = None
bandwidth = 1.5  # Bandwidth for Mean Shift
batch_size = 5
...
for batch in range(num_batches):
    ...
    if len(frames) > 1 and features.size > 0:
        # Use Mean Shift for prediction
        mean_shift = MeanShift(bandwidth=bandwidth)
        predicted_labels = mean_shift.fit_predict(features.reshape(features.shape[0], -1))
    ...
    # Display the tracked object using Mean Shift and linear regression predictions
    for i in range(len(frames)):
        if i == len(frames) - 1:
            break

        # Get the label of the last frame using Mean Shift
        mean_shift_label = predicted_labels[-1]
...
cv2.imwrite('original_vs_predicted_frames_meanshift.png', final_image)