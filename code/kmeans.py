...
for batch in range(num_batches):
    ...
    if len(frames) > 1 and features.size > 0:
        # Use K-Means clustering for prediction
    ...
    # Display the tracked object using K-Means clustering and linear regression predictions
    for i in range(len(frames)):
        if i == len(frames) - 1:
            break

        # Predict the next location using K-Means clustering
        kmeans_prediction = kmeans.predict(features[-1].reshape(1, -1))
...
cv2.imwrite('original_vs_predicted_frames_kmeans.png', final_image)